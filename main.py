...
async def process_pdf(
    ...
    document_id: str = Query(None, description="Optional document record ID")
):
    if file.content_type != "application/pdf":
        raise HTTPException(status_code=400, detail="Uploaded file must be a PDF.")

    if not initialize_documents_bucket():
        logging.error("Bucket initialization failed. Proceeding without initialization.")
    ...
    try:
        ...
        logging.info(f"Extracted {num_transactions} transaction(s).")
